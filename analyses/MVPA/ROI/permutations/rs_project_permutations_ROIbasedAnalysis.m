...
sem_index = [1, 5, 7, 9, 11, 13, 17, 19, 21];
nRuns=9;
condition= {'FACE_IDENTITY', 'PLACE_IDENTITY', 'FACE_PLACE', 'FACE', 'PLACE'};
niter=10000;
%% 1. Load real dataset
group_facePlace = load('accuracyGroup_FACE_PLACE_FAMILIARITY_sphere9_MVPA_N=23_nchoosek_leaveOneRunOut_LDA.mat');
...
for i=1:length(index)
    ...
    plot(xlim,[1/4 1/4], 'r');
    hold on;
    sem=[std(group_faceIdentity.accuracyGroup(:,sem_index(i)))/sqrt(length(subList)), ...
       std(group_placeIdentity.accuracyGroup(:,sem_index(i)))/sqrt(length(subList)); ...
       std(group_faceIdentity.accuracyGroup(:,sem_index(i)+1))/sqrt(length(subList)), ...
       std(group_placeIdentity.accuracyGroup(:,sem_index(i)+1))/sqrt(length(subList))];
...
for i=1:length(index)
    ...
    plot(xlim,[1/2 1/2], 'r');
    hold on;
    sem=[std(group_facePlace.accuracyGroup(:,sem_index(i)))/sqrt(length(subList)), ...
        std(group_face.accuracyGroup(:,sem_index(i)))/sqrt(length(subList))...
        std(group_place.accuracyGroup(:,sem_index(i)))/sqrt(length(subList)); ...
        std(group_facePlace.accuracyGroup(:,sem_index(i)+1))/sqrt(length(subList)), ...
        std(group_face.accuracyGroup(:,sem_index(i)+1))/sqrt(length(subList)),...
        std(group_place.accuracyGroup(:,sem_index(i)+1))/sqrt(length(subList))];
